...
public class CertService {
	...
	@Autowired
	private UnionPayAcpsdkProperties unionPayProperties;
	...
	private  void initSignCert() {
		if(!"01".equals(unionPayProperties.getSignMethod())){
			LogUtil.writeLog("非rsa签名方式，不加载签名证书。");
			return;
		}
		if (unionPayProperties.getSignCertPath() == null
				|| unionPayProperties.getSignCertPwd() == null
				|| unionPayProperties.getSignCertType() == null) {
		...
		try {
			keyStore = getKeyInfo(unionPayProperties.getSignCertPath(),
					unionPayProperties.getSignCertPwd(), unionPayProperties
							.getSignCertType());
	...
	private  void initMiddleCert() {
		LogUtil.writeLog("加载中级证书==>"+unionPayProperties.getMiddleCertPath());
		if (!isEmpty(unionPayProperties.getMiddleCertPath())) {
			middleCert = initCert(unionPayProperties.getMiddleCertPath());
	...
	private  void initRootCert() {
		LogUtil.writeLog("加载根证书==>"+unionPayProperties.getRootCertPath());
		if (!isEmpty(unionPayProperties.getRootCertPath())) {
			rootCert = initCert(unionPayProperties.getRootCertPath());
	...
	private  void initEncryptCert() {
		LogUtil.writeLog("加载敏感信息加密证书==>"+unionPayProperties.getEncryptCertPath());
		if (!isEmpty(unionPayProperties.getEncryptCertPath())) {
			encryptCert = initCert(unionPayProperties.getEncryptCertPath());
	...
	private  void initTrackKey() {
		if (!isEmpty(unionPayProperties.getEncryptTrackKeyModulus())
				&& !isEmpty(unionPayProperties.getEncryptTrackKeyExponent())) {
			encryptTrackKey = getPublicKey(unionPayProperties.getEncryptTrackKeyModulus(), 
					unionPayProperties.getEncryptTrackKeyExponent());
	...
	/*private  void initValidateCertFromDir() {
		...
		certMap.clear();
		String dir = unionPayProperties.getValidateCertDir();
	...
	public  PrivateKey getSignCertPrivateKey() {
		try {
			...
			PrivateKey privateKey = (PrivateKey) keyStore.getKey(keyAlias,
					unionPayProperties.getSignCertPwd().toCharArray());
			return privateKey;
	...
	public  PublicKey getEncryptCertPublicKey() {
		if (null == encryptCert) {
			String path = unionPayProperties.getEncryptCertPath();
			if (!isEmpty(path)) {
...
/*	public  PublicKey getValidatePublicKey(String certId) {
	...
	public  X509Certificate getMiddleCert() {
		if (null == middleCert) {
			String path = unionPayProperties.getMiddleCertPath();
			if (!isEmpty(path)) {
				initMiddleCert();
			} else {
	...
	public  X509Certificate getRootCert() {
		if (null == rootCert) {
			String path = unionPayProperties.getRootCertPath();
			if (!isEmpty(path)) {
				initRootCert();
			} else {
...
//			cert.verify(middleCert.getPublicKey());
		...
		/*if(unionPayProperties.isIfValidateCNName()){
			// 验证公钥是否属于银联
