...
import static com.zga.common.utils.SDKConstants.*;
...
public class SDKServiceUtil {
	...
	public  boolean sign(Map<String, String> data, String encoding) {
		...
		if (SIGNMETHOD_RSA.equals(signMethod)|| VERSION_1_0_0.equals(version) || VERSION_5_0_1.equals(version)) {
			if (VERSION_5_0_0.equals(version)|| VERSION_1_0_0.equals(version) || VERSION_5_0_1.equals(version)) {
				// 设置签名证书序列号
				data.put(SDKConstants.param_certId, certService.getSignCertId());
				...
				/*	byteSign = SecureUtil.base64Encode(SecureUtil.signBySoft(
							certService.getSignCertPrivateKey(), signDigest));*/
	...
	/*	else if (SIGNMETHOD_SHA256.equals(signMethod)) {
			return signBySecureKey(data, certService
	 ...
	 * @return 签名值
	 */
	public  boolean signBySecureKey(Map<String, String> data, String secureKey,
			String encoding) {
	...
	public  boolean signByCertInfo(Map<String, String> data,
		...
		if (SIGNMETHOD_RSA.equals(signMethod) || VERSION_1_0_0.equals(version) || VERSION_5_0_1.equals(version)) {
			if (VERSION_5_0_0.equals(version) || VERSION_1_0_0.equals(version) || VERSION_5_0_1.equals(version)) {
				// 设置签名证书序列号
				data.put(SDKConstants.param_certId, certService.getCertIdByKeyStoreMap(certPath, certPwd));
				// 将Map信息转换成key1=value1&key2=value2的形式
				String stringData = coverMap2String(data);
				LogUtil.writeLog("待签名请求报文串:[" + stringData + "]");
				byte[] byteSign = null;
				...
				try {
					...
					byte[] signDigest = SecureUtil
							.sha1X16(stringData, encoding);
					byteSign = SecureUtil.base64Encode(SecureUtil.signBySoft(
							certService.getSignCertPrivateKeyByStoreMap(certPath, certPwd), signDigest));
					stringSign = new String(byteSign);
					// 设置签名域值
					data.put(SDKConstants.param_signature, stringSign);
					return true;
	 ...
	 * @return
	 */
	public  boolean validateBySecureKey(Map<String, String> resData, String secureKey, String encoding) {
...
//				LogUtil.writeLog("验签公钥证书：["+strCert+"]");
				X509Certificate x509Cert = certService.genCertificateByStr(strCert);
				if(x509Cert == null) {
...
/*	public static int getEncryptCert(Map<String, String> resData,
		...
		if (CERTTYPE_01.equals(certType)) {
			// 更新敏感信息加密公钥
			if (!certService.getEncryptCertId().equals(
				...
				certService.resetEncryptCertPublicKey();
				return 1;
			}else {
				return 0;
			}
...
//			// 更新磁道加密公钥
//			if (!certService.getEncryptTrackCertId().equals(
//					x509Cert.getSerialNumber().toString())) {
//				// ID不同时进行本地证书更新操作
...
//					return -1;
//				LogUtil.writeLog("save new encryptPubKeyCert success");
//				certService.resetEncryptTrackCertPublicKey();
//				return 1;
//			}else {
				return 0;
//			}
